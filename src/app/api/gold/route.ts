...
const token = process.env.X_SHOPIFY_ACCESS_TOKEN;
  ...
  // console.log(productsdata);
    ...
    updateProductPrice(id, goldPriceOneGram);
  }
...
async function updateProductPrice(Id: { productId: number; variantId: number }, newPrice: number) {

const token = process.env.X_SHOPIFY_ACCESS_TOKEN;


  const productId = Id.productId;
  const variantId = Id.variantId;
  ...
  const makingCharge = +metafieldsdata.metafields.find(
  ...
  const goldPriceparsentChange = ((newPrice - 561.54) / 561.54) * 100;

  const rowGoldPrice = newPrice * goldWeight;

  const newTotalPrice = +(
    rowGoldPrice +
    (makingCharge + (makingCharge * goldPriceparsentChange) / 100)
  ).toFixed(2);
